...
class BleGattManager(
    ...
    fun connect() {
        ...
        if (!savedMac.isNullOrEmpty() && BluetoothAdapter.checkBluetoothAddress(savedMac)) {
            log("Tentativo di connessione diretta al MAC salvato: $savedMac...")
            try {
    ...
    private fun startLeScan(adapter: BluetoothAdapter) {
        ...
        log("Avvio scansione BLE per Livall BR80 / BlingRemote...")
        isScanning = true
        ...
        val callback = object : ScanCallback() {
            override fun onScanResult(callbackType: Int, result: ScanResult) {
                ...
                val matchesMac = (savedMac != null && device.address.equals(savedMac, ignoreCase = true))

                if (matchesName || matchesService || matchesMac) {
                    val displayName = recordName ?: devName ?: "BR80"
                    log("Trovato device: $displayName [${device.address}], RSSI: ${result.rssi}")
                    stopLeScan()
